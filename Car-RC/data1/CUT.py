...
def threshold_demo_3(image):
    img_thre = image
    cv2.threshold(image, 195, 255, cv2.THRESH_BINARY_INV, img_thre)
...
def find_end(start, arg, black, white, width, black_max, white_max):
    ...
    for m in range(start + 1, width - 1):
        if (black[m] if arg else white[m]) > (0.99999 * black_max if arg else 0.99999 * white_max):
            end = m
            break
    return end
...
def split(thresh, name):
    ...
    while n < width - 1:
        ...
        if (white[n] if arg else black[n]) > (0.00001 * white_max if arg else 0.00001 * black_max):
            start = n
